...
### B. API Level (Server)
- **JWT Session Cookies**: API sessions are issued as `HttpOnly` cookies so browser clients do not persist bearer tokens in readable local storage or app caches.
...
- **CORS Policy**: Only explicitly allowed local/web origins can call authenticated routes with credentials enabled.
...
- **Release Guardrail**: Production deployments reject non-HTTPS requests unless explicitly disabled for local development.
