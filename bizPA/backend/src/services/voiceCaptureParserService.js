...
function endOfWeek(referenceDate) {
  const result = startOfWeek(referenceDate);
  result.setDate(result.getDate() + 6);
  result.setHours(23, 59, 59, 999);
  return result;
}

function parseTimePeriodHint(text, referenceDate) {
  const now = new Date(referenceDate);
  ...
  if (/\btoday\b/i.test(text)) {
    return {
      key: 'today',
      raw: 'today',
      start_iso: toIsoDate(now),
      end_iso: toIsoDate(now)
    };
  }
  ...
  if (/\bthis week\b/i.test(text)) {
    ...
    return {
      key: 'this_week',
      raw: 'this week',
      start_iso: toIsoDate(start),
      end_iso: toIsoDate(end)
    };
  }
...
function trimEntityValue(value) {
  ...
  return value
    .replace(/\b(?:this|last)\s+week\b/gi, '')
...
function extractCounterpartyName(text, intent) {
  ...
  for (const pattern of patterns) {
    ...
    const value = trimEntityValue(
      match[1]
        .split(/\b(?:for|at|on|due|tomorrow|today|yesterday|next|this\s+week|last\s+week|\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}(?:\/\d{2,4})?)\b/i)[0]
        .trim()
    );
...
function buildCaptureParseResult(transcript, clientDateStr = null) {
  ...
  const timePeriodHint = parseTimePeriodHint(text, referenceDate);
...
function toLegacyIntentShape(parseResult) {
  ...
  if (parseResult.time_period?.key) {
    slots.time_period = parseResult.time_period.key;
    slots.time_period_hint = parseResult.time_period;
  }
