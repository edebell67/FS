  ...
  try {
    const query = `
      SELECT
        ...
        jsonb_build_object(
          'category_code', rules.category_code,
          'category_name', rules.category_name,
          'default_business_personal', rules.default_business_personal,
          'default_split_business_pct', rules.default_split_business_pct
      ...
      LEFT JOIN LATERAL (
        SELECT
          r.category_code,
          r.category_name,
          r.default_business_personal,
          r.default_split_business_pct
        FROM rules r
        WHERE r.user_id = bt.user_id
          AND r.is_active = TRUE
          AND bt.merchant IS NOT NULL
          AND bt.merchant ILIKE '%' || r.merchant_pattern || '%'
        ORDER BY length(r.merchant_pattern) DESC, r.updated_at DESC
        LIMIT 1
      ) rules ON TRUE
      WHERE bt.user_id = $1
        AND (
...
const classifyTransaction = async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    const upserted = await upsertTransactionClassification(client, {
      userId,
      bankTxnId,
      updates: {
        category_code,
      ...
      auditSource: source === 'manual' ? 'ui' : source,
    });
