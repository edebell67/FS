...
} = require('../services/quarterlyExportService');
const {
  ...
  try {
    ...
    const evidenceRows = await db.query(
      ...
      SELECT
        ...
        state.bank_txn_id AS matched_bank_txn_id,
        COALESCE(state.user_confirmed, false) AS user_confirmed
      FROM evidence e
      LEFT JOIN LATERAL (
        SELECT el.bank_txn_id, el.user_confirmed, el.confirmed_at, el.created_at
    ...
    archive.append(quarterlyPackReport.pdf, { name: 'QuarterlyPack.pdf' });
