/*
 * Ridgeline Motor Works — demo site interactions.
...
(function () {
  ...
  function initCookieBanner() {
    ...
    if (!banner) return;
    if (localStorage.getItem("ridgeline_cookie_choice")) return;
    setTimeout(() => banner.classList.add("show"), 900);
    document.getElementById("cookieAccept")?.addEventListener("click", () => {
      localStorage.setItem("ridgeline_cookie_choice", "accepted");
      banner.classList.remove("show");
    });
    document.getElementById("cookieDecline")?.addEventListener("click", () => {
      localStorage.setItem("ridgeline_cookie_choice", "declined");
      banner.classList.remove("show");
    });
  }
