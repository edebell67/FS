...
2. [Endpoints](#2-endpoints) *(includes 2.6 GET /strategy-types)*
...
| `type`      | string  | *(all)*    | Filter by strategy type — use exact string value from `GET /strategy-types` e.g. `RSI Mean Rev` |
...
| `type`                   | string    | Strategy type — free-form string as returned by `GET /strategy-types` e.g. `"Breakout v2"`, `"RSI Mean Rev"` |
...
### 2.6 `GET /strategy-types` — Available Strategy Types

Returns the full list of strategy type values present in the system. The dashboard fetches this once at load time to dynamically build the type filter dropdown. Values are free-form strings exactly as stored against each strategy.

| | |
|---|---|
| **Method** | `GET` |
| **Path** | `/v1/strategy-types` |
| **Auth** | `X-API-Key` header required |
| **Cache TTL** | 5 minutes (types change infrequently) |

#### Response — `data.types[]`

A simple array of unique type strings, sorted alphabetically:
...
> 📌 **Dashboard behaviour:** The type filter dropdown is empty until this endpoint responds. If the endpoint fails, the filter is hidden and the dashboard falls back to showing all strategies unfiltered. Type strings are rendered in the UI exactly as returned — no mapping, no transformation.
...
### 4.1 Strategy Types — Dynamic from API

Strategy types are **not hardcoded in the dashboard**. The full list is fetched at load time from `GET /v1/strategy-types` (section 2.6) and the filter UI is built dynamically from whatever the API returns.

Type values are **free-form strings** exactly as stored in your platform — e.g. `"Breakout v2"`, `"RSI Mean Rev"`, `"EMA Crossover"`. The dashboard renders them verbatim with no mapping or transformation.

The `type` query parameter on `GET /strategies` uses the exact string value from the types endpoint:

```http
# Step 1 — fetch available types on dashboard load
GET /v1/strategy-types
→ { "types": ["Breakout v2", "RSI Mean Rev", "EMA Crossover", ...] }

# Step 2 — user selects a type, dashboard fires filtered request
GET /v1/strategies?type=Breakout+v2&sort=pnl_desc
```

> ⚠️ **Consistency requirement:** The same strategy type must always return the exact same string from the API. Case and spacing are significant. Renaming a type in the platform will automatically update the filter — no dashboard changes needed.
...
```http
GET /v1/strategies?limit=50&sort=winrate_desc&type=RSI+Mean+Rev&pair=GBPUSD&min_trades=10
X-API-Key: sk-live-abc123
```
...
|------------------|---------|------------------|----------------|
| **Leaderboard**  | `GET /strategies?limit=20&sort=pnl_desc` + `GET /strategy-types` | `GET /strategies?type=<value>&pair=Y` | Drill-down → `GET /strategies/:id/trades` |
...
| `GET /strategy-types`             | 10 req/min   | 5 minutes             |
...
| 1.1     | 27 Feb 2026 | FXPilot Team  | Strategy types changed to dynamic API-driven free-form strings; added GET /strategy-types endpoint |
