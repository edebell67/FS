...
This skill adds only the handyman-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: practical, dependable tones (workwear blue / amber accent / neutral). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Handyman

- **Services**: repairs, shelving, flat-pack assembly, decorating, doors, fixtures, sealant, small plumbing, small electrical work only where qualified, and property maintenance.
- **Qualification data** (feeds the quote form and AI qualification): task list, photos, materials, access, number of visits, urgency, and property type.
- **Trust signals**: clear scope boundaries, hourly or fixed pricing, insurance, qualifications, materials policy, and tidy working. Use only honest, non-fabricated differentiators.
- **FAQs**: minimum charge, materials, estimates, urgent work, multiple tasks, and jobs outside scope.

## 5. Imagery — map the blueprint image matrix to handyman

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Handyman-specific subjects for each slot:

- **Hero (1)**: a handyman at work on a home repair (tidy, professional).
- **Two split/feature sections (1 each)**: **Repairs & fixtures** (repairs, shelving, doors, fixtures, sealant, flat-pack assembly) and **Maintenance & finishing** (decorating, small plumbing, property maintenance, odd jobs), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: varied small projects grouped by room and task.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `handyman`, `home-repair`, `tool-belt`, `diy-fixing`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Send a Job List**, **Upload Photos**, and **Book a Home Visit**. Clearly exclude regulated work unless the business is qualified. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
