...
This skill adds only the barbers-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: classic barbershop tones (deep red / charcoal / warm neutral); avoid the softer rose already used by the nail bar build. Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Barbers

- **Services**: skin fades, classic cuts, beard trims, hot towel shaves, restyling, children's cuts, and grooming packages.
- **Booking data** (feeds the quote form and AI qualification): preferred barber, service, date/time, new or returning client, and cancellation policy.
- **Trust signals**: barber profiles, qualifications, hygiene standards, prices, opening hours, walk-ins, and accessible services. Use only honest, non-fabricated differentiators.
- **FAQs**: consultation, children, late arrivals, deposits, products, and aftercare.

## 5. Imagery — map the blueprint image matrix to barbers

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Barbers-specific subjects for each slot:

- **Hero (1)**: a barber chair or shop interior (reads best), or a sharp finished cut.
- **Two split/feature sections (1 each)**: **Barbering** (fades, classic cuts, beard work, hot towel shaves) and **Grooming & styling** (restyles, kids' cuts, grooming packages, products), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: cuts by style, hair length and texture, beard work, hot-towel detail, and before/after transformations.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `barbershop`, `barber-chair`, `beard-trim`, `mens-haircut`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Book an Appointment**, **Call the Shop**, and **Get Directions**. Show live booking or a clear callback route, service duration, price, and an availability disclaimer. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
