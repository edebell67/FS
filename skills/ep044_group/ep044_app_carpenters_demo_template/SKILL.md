...
This skill adds only the carpenters-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: warm natural-timber tones (walnut / oak / forest green accent). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Carpenters

- **Services**: fitted wardrobes, built-in storage, doors, staircases, cabinetry, shelving, flooring, roofing timber, repairs, and bespoke furniture.
- **Qualification data** (feeds the quote form and AI qualification): drawings or photos, dimensions, materials, finish, access, property type, and installation deadline.
- **Trust signals**: workshop capability, materials, finish quality, dust protection, insurance, and project management. Use only honest, non-fabricated differentiators.
- **FAQs**: site survey, lead times, painting, materials, structural work, and variations.

## 5. Imagery — map the blueprint image matrix to carpenters

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Carpenters-specific subjects for each slot:

- **Hero (1)**: a beautifully finished piece of bespoke joinery (fitted wardrobe, staircase, or alcove units).
- **Two split/feature sections (1 each)**: **Bespoke joinery** (fitted wardrobes, alcove units, cabinetry, bespoke furniture) and **Structural & repairs** (doors, staircases, flooring, roofing timber, repairs), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: fitted furniture, kitchens, staircases, alcove units, trim and mouldings, and repair transformations.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `fitted-wardrobe`, `bespoke-joinery`, `wooden-staircase`, `carpentry-workshop`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Request a Site Visit**, **Upload Your Plans**, and **Get a Bespoke Quote**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
