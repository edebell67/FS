...
This skill adds only the bathroom fitters-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: calm, clean spa tones (deep teal / slate / warm stone). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Bathroom Fitters

- **Services**: bathroom renovations, wet rooms, walk-in showers, tiling, plumbing, electrics, underfloor heating, accessible bathrooms, and full suite installation.
- **Qualification data** (feeds the quote form and AI qualification): room type, dimensions, bath or shower preference, plumbing changes, finish, budget, postcode, and target date.
- **Trust signals**: project management, workmanship guarantee, insurance, waste removal, estimated duration, and brands supplied. Use only honest, non-fabricated differentiators.
- **FAQs**: design process, disruption, waterproofing, building regulations, ventilation, payment stages, and who supplies materials.

## 5. Imagery — map the blueprint image matrix to bathroom fitters

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Bathroom Fitters-specific subjects for each slot:

- **Hero (1)**: a finished premium bathroom (walk-in shower or freestanding bath).
- **Two split/feature sections (1 each)**: **Design & supply** (layouts, suites, tiling and finishes, brands supplied) and **Installation** (plumbing, electrics, underfloor heating, waterproofing, project management), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: modern, luxury, family, ensuite, compact, wet-room and marble projects, plus before-and-after renovations.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `modern-bathroom`, `walk-in-shower`, `bathroom-tiling`, `wet-room`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Book a Free Design Visit**, **Request a Bathroom Quote**, and **View Bathroom Inspiration**. Offer image upload and a structured AI consultation. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
