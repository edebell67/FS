...
This skill adds only the fencing-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: natural outdoor tones (timber brown / deep green / stone). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Fencing

- **Services**: panel fencing, closeboard, featheredge, concrete posts, gates, trellis, repairs, replacement, and commercial fencing.
- **Qualification data** (feeds the quote form and AI qualification): length, height, ground conditions, access, old-fence removal, gate requirements, and boundary permissions.
- **Trust signals**: materials, posts and foundations, waste removal, guarantees, neighbour coordination, and insurance. Use only honest, non-fabricated differentiators.
- **FAQs**: planning, party boundaries, weather, maintenance, lead time, and payment.

## 5. Imagery — map the blueprint image matrix to fencing

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Fencing-specific subjects for each slot:

- **Hero (1)**: a newly installed, well-finished fence line or garden gate.
- **Two split/feature sections (1 each)**: **Fencing** (panel, closeboard, featheredge, concrete posts, commercial fencing) and **Gates & extras** (gates, trellis, repairs, replacement, waste removal), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: timber, composite, decorative and security fencing, gates, and before-and-after gardens.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `garden-fence`, `wooden-fence`, `fence-installation`, `garden-gate`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Request a Free Measure**, **Upload Garden Photos**, and **Get a Fencing Quote**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
