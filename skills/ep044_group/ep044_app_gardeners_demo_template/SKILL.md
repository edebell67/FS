...
This skill adds only the gardeners-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: fresh natural tones (deep garden green / terracotta / warm stone). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Gardeners

- **Services**: garden maintenance, lawn care, hedge trimming, planting, pruning, clearance, patios, raised beds, fencing, and landscaping.
- **Qualification data** (feeds the quote form and AI qualification): garden size, goals, access, waste, planting preferences, maintenance frequency, and budget.
- **Trust signals**: horticultural experience, waste credentials, equipment, seasonal planning, insurance, and maintenance plans. Use only honest, non-fabricated differentiators.
- **FAQs**: visits, weather, waste removal, pesticides, design process, and ongoing contracts.

## 5. Imagery — map the blueprint image matrix to gardeners

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Gardeners-specific subjects for each slot:

- **Hero (1)**: a lush, well-kept garden or a striking planted border.
- **Two split/feature sections (1 each)**: **Garden maintenance** (lawn care, hedge trimming, pruning, planting, clearance) and **Landscaping** (patios, raised beds, fencing, design-led transformations), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: seasonal work, planting schemes, lawns, patios, borders, and full garden transformations.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `landscaped-garden`, `garden-lawn`, `flower-border`, `patio-garden`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Book a Garden Visit**, **Send Garden Photos**, and **Plan My Garden**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
