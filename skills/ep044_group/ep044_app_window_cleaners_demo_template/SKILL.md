...
This skill adds only the window cleaners-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: bright, clean tones (crisp sky blue / white / fresh neutral). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Window Cleaners

- **Services**: traditional window cleaning, water-fed pole, conservatories, gutters, solar panels, commercial windows, builders cleans, and one-off cleans.
- **Qualification data** (feeds the quote form and AI qualification): property type, window count, height, access, frequency, frames, conservatory, parking, and preferred date.
- **Trust signals**: insurance, safe systems of work, equipment reach, water treatment, recurring schedules, and weather policy. Use only honest, non-fabricated differentiators.
- **FAQs**: frequency, access, frames, rain, security, payment, and missed visits.

## 5. Imagery — map the blueprint image matrix to window cleaners

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Window Cleaners-specific subjects for each slot:

- **Hero (1)**: sparkling clean windows on an attractive property (or water-fed pole work).
- **Two split/feature sections (1 each)**: **Domestic & recurring** (traditional and water-fed cleaning, conservatories, regular rounds) and **Commercial & extras** (commercial windows, solar panels, gutters, builders cleans), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: clear windows, conservatories, commercial frontage, and difficult-access examples.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `clean-windows`, `window-cleaner`, `water-fed-pole`, `glass-cleaning`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Get a Window Cleaning Quote**, **Set Up a Regular Service**, and **Book a One-Off Clean**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
