...
This skill adds only the guttering-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: clean, dependable tones (slate blue / grey / bright neutral). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Guttering

- **Services**: gutter cleaning, repairs, replacement, fascia and soffit, downpipes, gutter guards, inspections, and commercial maintenance.
- **Qualification data** (feeds the quote form and AI qualification): property height, gutter type, symptoms, access, leaks, photos, and urgency.
- **Trust signals**: safe access methods, insurance, waste disposal, materials, guarantees, and before/after evidence. Use only honest, non-fabricated differentiators.
- **FAQs**: access, frequency, weather, repairs versus replacement, pricing, and water damage.

## 5. Imagery — map the blueprint image matrix to guttering

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Guttering-specific subjects for each slot:

- **Hero (1)**: clean, well-maintained guttering on an attractive property (or safe access work).
- **Two split/feature sections (1 each)**: **Cleaning & inspection** (gutter cleaning, inspections, gutter guards, maintenance) and **Repair & replacement** (repairs, replacement, fascia, soffit, downpipes), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: cleaned systems, repaired joints, fascia replacement, and compliant installations.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `house-guttering`, `roof-gutter`, `gutter-cleaning`, `fascia-soffit`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Book a Gutter Inspection**, **Get a Cleaning Quote**, and **Send Photos**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
