...
This skill adds only the driving instructors-specific content and imagery below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies. A full driving instructors website is expected: every common page above, populated with meaningful, category-accurate content.
...
- **Palette**: approachable, confident tones (fresh green / calm blue / bright neutral). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep for stray hex outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Driving Instructors

- **Services**: manual and automatic lessons, beginner courses, intensive/semi-intensive courses, Pass Plus, refresher and motorway lessons, and theory support.
- **Enquiry & lesson data** (feeds the quote/booking form and AI qualification): licence stage (none/provisional/full), manual or automatic, current experience, test booked?, preferred area, availability, and goal date.
- **Trust signals**: DVSA-approved (ADI) status where genuine, grade, years teaching, dual-control car, patient approach, areas covered, and honest pass information. Use only honest, non-fabricated differentiators.
- **FAQs**: lesson length and pricing, manual vs automatic, block booking, cancellations, test-day car use, and how many lessons are typical.
- **Business model note**: a service-area business with no shopfront — lead with the areas/postcodes covered and availability, not a premises. There is no 'completed work' gallery: use car/lesson/pass imagery and a coverage area instead of a project gallery, and skip the Before & After section.

## 5. Imagery — map the blueprint image matrix to driving instructors

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load, descriptive alt text). Driving Instructors-specific subjects:

- **Hero (1)**: a friendly instructor with a learner beside a dual-control car, or a learner driving confidently.
- **Two split/feature sections (1 each)**: **Learning to drive** (beginner lessons, manual and automatic, structured lesson plans) and **Courses & progression** (intensive courses, Pass Plus, refresher and motorway lessons, theory support), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: dual-control learner cars, lessons in progress, instructor with a learner, L-plates, and celebration/pass moments (with permission — never fabricate pass rates).
- **Before & After**: not applicable to this category — omit the Before & After section rather than fabricating pairs.
- **Search queries** for sourcing: `driving-lesson`, `learner-driver`, `driving-instructor`, `driving-school-car`. Vary composition (wide/detail, in-action).

## 6. Conversion

Category CTAs: **Book Your First Lesson**, **Check Availability in Your Area**, and **Ask About Courses**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
