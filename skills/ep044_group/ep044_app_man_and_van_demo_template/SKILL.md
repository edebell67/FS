...
This skill adds only the man and van-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: reliable, energetic tones (bold blue / orange accent / neutral). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Man and Van

- **Services**: house moves, single-item delivery, furniture collection, student moves, office moves, clearances, storage moves, and courier work.
- **Qualification data** (feeds the quote form and AI qualification): collection and delivery postcodes, item list, floors, lifts, parking, access, dates, helpers, and insurance needs.
- **Trust signals**: vehicle size, goods-in-transit cover, cancellation terms, punctuality, equipment, and prohibited items. Use only honest, non-fabricated differentiators.
- **FAQs**: pricing basis, stairs, packing, waiting time, deposits, long-distance work, and cancellations.

## 5. Imagery — map the blueprint image matrix to man and van

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Man and Van-specific subjects for each slot:

- **Hero (1)**: a clean van being loaded, or the team carefully moving furniture.
- **Two split/feature sections (1 each)**: **Moves & removals** (house moves, office moves, student moves, storage moves) and **Deliveries & clearances** (single-item delivery, furniture collection, clearances, courier work), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: vehicles, careful loading, furniture protection, and the team at work.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `removal-van`, `moving-house`, `furniture-moving`, `man-with-van`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Get a Moving Quote**, **Check Availability**, and **Send an Item List or Photos**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
