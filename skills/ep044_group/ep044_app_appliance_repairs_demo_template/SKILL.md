...
This skill adds only the appliance repairs-specific content and imagery below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies. A full appliance repairs website is expected: every common page above, populated with meaningful, category-accurate content.
...
- **Palette**: practical, dependable tones (trust blue / graphite / clean neutral). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep for stray hex outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Appliance Repairs

- **Services**: washing machine, tumble dryer, dishwasher, oven and cooker, fridge and freezer, and integrated-appliance repairs, plus diagnostics and part replacement.
- **Qualification data** (feeds the quote/booking form and AI qualification): appliance type and brand, model number, fault symptoms, age, error codes, photos, postcode, and preferred visit time.
- **Trust signals**: manufacturer/brand experience, genuine parts, guarantees on repairs, insurance, call-out and diagnosis pricing, and fixed-first-visit rate where offered. Use only honest, non-fabricated differentiators.
- **FAQs**: call-out charge, same-day repair, guarantees, genuine parts, brands covered, when to repair vs replace, and warranty impact.
- **Business model note**: mobile / we-come-to-you service — lead with a coverage area, call-out terms and response time rather than a shopfront; a clear service-area list/map replaces a premises gallery.

## 5. Imagery — map the blueprint image matrix to appliance repairs

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load, descriptive alt text). Appliance Repairs-specific subjects:

- **Hero (1)**: an engineer professionally repairing a home appliance (washing machine or oven).
- **Two split/feature sections (1 each)**: **Laundry & kitchen appliances** (washing machines, tumble dryers, dishwashers, ovens and cookers) and **Refrigeration & integrated** (fridges, freezers, integrated appliances, diagnostics and part replacement), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: engineers at work, common appliances being repaired, replaced parts, and tidy completed repairs (no misleading claims).
- **Before & After**: honest broken-vs-working pairs only where genuinely photographable (e.g. a cleaned/repaired appliance) — never staged.
- **Search queries** for sourcing: `appliance-repair`, `washing-machine-repair`, `oven-repair`, `refrigerator-technician`. Vary composition (wide/detail, in-action).

## 6. Conversion

Category CTAs: **Book a Repair Visit**, **Get a Diagnosis Quote**, and **Send Photos of the Fault**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
