...
This skill adds only the kitchen fitters-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: premium, warm tones (deep green / brass accent / warm stone). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Kitchen Fitters

- **Services**: kitchen fitting, units, worktops, appliances, plumbing, electrics, tiling, flooring, splashbacks, refurbishment, and accessibility adaptations.
- **Qualification data** (feeds the quote form and AI qualification): room dimensions, layout, appliances, structural changes, style, budget, and target completion date.
- **Trust signals**: design coordination, trades management, installation warranty, appliance handling, waste removal, and suppliers. Use only honest, non-fabricated differentiators.
- **FAQs**: design process, lead times, temporary kitchen, plumbing and electrics, worktops, payment stages, and snagging.

## 5. Imagery — map the blueprint image matrix to kitchen fitters

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Kitchen Fitters-specific subjects for each slot:

- **Hero (1)**: a beautifully finished fitted kitchen.
- **Two split/feature sections (1 each)**: **Design & supply** (layouts, units, worktops, appliances, style) and **Installation** (fitting, plumbing, electrics, tiling, flooring, splashbacks), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: contemporary, traditional, compact and utility kitchens, worktops, storage, and before-and-after projects.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `modern-kitchen`, `fitted-kitchen`, `kitchen-worktop`, `kitchen-installation`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Book a Kitchen Design Visit**, **Upload Your Plans**, and **Request a Kitchen Quote**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
