...
This skill adds only the bricklayers-specific content and imagery below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies. A full bricklayers website is expected: every common page above, populated with meaningful, category-accurate content.
...
- **Palette**: earthy, solid tones (brick terracotta / charcoal / warm stone). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep for stray hex outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Bricklayers

- **Services**: new build brickwork, extensions, garden and boundary walls, repointing, feature brickwork, block and stonework, chimney rebuilds, and structural repairs.
- **Qualification data** (feeds the quote/booking form and AI qualification): project type, dimensions, brick/block/stone preference, existing structure, drawings or photos, access, and target dates.
- **Trust signals**: craftsmanship and finish quality, materials, foundations and coursing, building-regs awareness, insurance, guarantees, and project management. Use only honest, non-fabricated differentiators.
- **FAQs**: planning and building regs, lead times, weather, matching existing brick, repointing vs rebuild, and payment stages.

## 5. Imagery — map the blueprint image matrix to bricklayers

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load, descriptive alt text). Bricklayers-specific subjects:

- **Hero (1)**: a crisply finished, well-coursed brick wall or a completed brick extension.
- **Two split/feature sections (1 each)**: **New build & structural** (extensions, new brickwork, block and stonework, structural work) and **Garden & restoration** (boundary and garden walls, repointing, feature brickwork, chimney rebuilds), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: completed walls, extensions, feature brickwork, repointing detail, arches and piers, and before-and-after repairs.
- **Before & After**: strong fit for this category — original condition vs completed brickwork (e.g. tired repointing → crisp new joints).
- **Search queries** for sourcing: `brickwork`, `bricklayer`, `brick-wall`, `brick-extension`. Vary composition (wide/detail, in-action).

## 6. Conversion

Category CTAs: **Request a Site Visit**, **Upload Your Plans or Photos**, and **Get a Brickwork Quote**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
