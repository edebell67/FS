...
This skill adds only the dog grooming-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: friendly, warm tones (soft teal / warm cream / playful accent). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Dog Grooming

- **Services**: full groom, bath and blow-dry, breed trims, puppy introductions, nail clipping, deshedding, teeth cleaning, and hand stripping where offered.
- **Booking data** (feeds the quote form and AI qualification): breed, age, coat condition, temperament, health needs, matting, vaccination policy, and emergency contact.
- **Trust signals**: groomer experience, welfare policy, handling approach, salon hygiene, insurance, and collection arrangements. Use only honest, non-fabricated differentiators.
- **FAQs**: first visit, anxious dogs, matting, duration, vaccinations, pricing, and late collection.

## 5. Imagery — map the blueprint image matrix to dog grooming

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Dog Grooming-specific subjects for each slot:

- **Hero (1)**: a happy, freshly groomed dog (or a groomer gently working with a dog).
- **Two split/feature sections (1 each)**: **Grooming & styling** (full grooms, breed trims, bath and blow-dry, hand stripping) and **Care & welfare** (puppy introductions, nail clipping, deshedding, anxious-dog handling), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: breed and coat examples, before-and-after grooms, and realistic, welfare-first outcomes.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `dog-grooming`, `groomed-dog`, `dog-salon`, `puppy-bath`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Book a Grooming Appointment**, **Request a Coat Assessment**, and **Call About Your Dog**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
