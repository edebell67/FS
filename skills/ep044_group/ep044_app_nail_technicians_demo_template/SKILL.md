...
This skill adds only the nail technicians-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: refined beauty tones distinct from the existing rose nail bar (consider mauve / plum / warm nude — check the registry). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Nail Technicians

- **Services**: manicures, gel, BIAB, extensions, acrylics, nail art, infills, removals, pedicures, and bridal or event packages.
- **Booking data** (feeds the quote form and AI qualification): service, nail length, existing product, design reference, allergies, date/time, and deposit policy.
- **Trust signals**: training, hygiene and sterilisation, product brands, patch tests where relevant, and aftercare. Use only honest, non-fabricated differentiators.
- **FAQs**: removal, infills, durability, repairs, allergies, deposits, late arrivals, and children.

## 5. Imagery — map the blueprint image matrix to nail technicians

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Nail Technicians-specific subjects for each slot:

- **Hero (1)**: a beautifully finished set of nails or a styled salon interior.
- **Two split/feature sections (1 each)**: **Nails & enhancements** (manicures, gel, BIAB, extensions, acrylics, infills) and **Art & occasions** (nail art, seasonal designs, pedicures, bridal and event packages), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: shape, length and colour, seasonal designs, detailed nail art, and natural finishes.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `manicure`, `nail-art`, `gel-nails`, `nail-salon`. Vary composition (wide/detail, premises/in-action).

### Mandatory private-concept visual fallback

A nail business is an inherently visual service. A gallery made of empty category cards, placeholders or phrases such as “reserved for verified evidence” is **not acceptable**, even in a private review. When owner-approved work is unavailable, use a curated set of real, verified stock manicure/nail-art/studio-process images to demonstrate the *visual direction*:

1. Show one nail image in the homepage hero or primary visual area, a minimum four-image home preview strip, and a five-or-more image dedicated gallery; prefer the blueprint's 9–10 editorial-gallery target when source coverage allows.
2. Put an unavoidable label near the visual set: `ILLUSTRATIVE NAIL DIRECTION — NOT VERIFIED WORK BY <BUSINESS>`.
3. Use useful visual-direction captions (for example, soft colour, natural finish, statement art, studio process), never captions implying that the named business performed a treatment or owns the image.
4. Keep a provenance record for every asset and replace the images with owner-approved work before official launch.
5. Verify every image response/load and browser-review the home and gallery routes before publishing. Never leave a visually led category image-free because project evidence is still pending.

## 6. Conversion

Category CTAs: **Book Your Appointment**, **View Nail Designs**, and **Send a Design Reference**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
