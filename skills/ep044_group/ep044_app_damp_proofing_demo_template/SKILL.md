...
This skill adds only the damp proofing-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: reassuring, technical tones (deep navy / slate blue / clean neutral). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Damp Proofing

- **Services**: rising damp, penetrating damp, condensation, mould treatment, tanking, damp surveys, timber treatment, and basement waterproofing.
- **Qualification data** (feeds the quote form and AI qualification): symptoms, affected rooms, property age, previous work, photographs, tenure, and urgency.
- **Trust signals**: survey methodology, qualifications, guarantees, insurance, ventilation advice, and referral of structural issues. Use only honest, non-fabricated differentiators.
- **FAQs**: causes, survey process, drying time, replastering, guarantees, and when to involve a specialist.

## 5. Imagery — map the blueprint image matrix to damp proofing

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Damp Proofing-specific subjects for each slot:

- **Hero (1)**: a professional carrying out a damp survey, or a completed dry, replastered wall.
- **Two split/feature sections (1 each)**: **Survey & diagnosis** (rising/penetrating damp, condensation, moisture surveys, written reports) and **Remediation** (tanking, timber treatment, mould treatment, basement waterproofing), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: survey findings and completed remediation, presented honestly without overstating outcomes.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `damp-survey`, `damp-proofing`, `basement-waterproofing`, `wall-plastering`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Book a Damp Survey**, **Send Photos for Initial Advice**, and **Request a Written Report**. The AI assistant must not diagnose on its own. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
