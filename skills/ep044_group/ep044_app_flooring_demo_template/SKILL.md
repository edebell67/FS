...
This skill adds only the flooring-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: warm, material-led tones (natural oak / charcoal / warm neutral). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Flooring

- **Services**: hardwood, engineered wood, laminate, LVT, vinyl, carpet, screeding, subfloor preparation, sanding, and repairs.
- **Qualification data** (feeds the quote form and AI qualification): room measurements, existing floor, subfloor, moisture, underfloor heating, preferred finish, and occupied status.
- **Trust signals**: product knowledge, moisture testing, acclimatisation, preparation, fitting warranty, and waste handling. Use only honest, non-fabricated differentiators.
- **FAQs**: fitting time, furniture, levelling, cleaning, maintenance, samples, and aftercare.

## 5. Imagery — map the blueprint image matrix to flooring

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Flooring-specific subjects for each slot:

- **Hero (1)**: a beautifully finished floor in a styled room (wood or LVT reads best).
- **Two split/feature sections (1 each)**: **Wood & laminate** (hardwood, engineered wood, laminate, sanding and refinishing) and **Resilient & carpet** (LVT, vinyl, carpet, subfloor preparation and screeding), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: material-led projects, patterns, transitions, stairs, and refinishing transformations.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `hardwood-floor`, `wood-flooring`, `laminate-floor`, `interior-flooring`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Book a Measure**, **Order or View Samples**, and **Get a Flooring Quote**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
