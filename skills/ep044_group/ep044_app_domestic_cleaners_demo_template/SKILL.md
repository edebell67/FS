...
This skill adds only the domestic cleaners-specific content below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies.
...
- **Palette**: fresh, trustworthy tones (clean green / sky blue / bright neutral). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep the finished file for stray hex values outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Domestic Cleaners

- **Services**: regular cleaning, deep cleans, end-of-tenancy, oven cleaning, windows, laundry, ironing, and post-build cleaning.
- **Qualification data** (feeds the quote form and AI qualification): property type, rooms, frequency, preferred products, pets, access, parking, and required date.
- **Trust signals**: DBS or vetting policy where applicable, insurance, key handling, products, checklists, and team identity. Use only honest, non-fabricated differentiators.
- **FAQs**: supplies, first clean, cancellations, minimum hours, recurring visits, and satisfaction policy.

## 5. Imagery — map the blueprint image matrix to domestic cleaners

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load). Domestic Cleaners-specific subjects for each slot:

- **Hero (1)**: a bright, spotless living space or kitchen.
- **Two split/feature sections (1 each)**: **Regular cleaning** (weekly/fortnightly visits, trusted teams, checklists) and **Deep & specialist** (deep cleans, end-of-tenancy, oven, post-build cleaning), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: clear examples of kitchens, bathrooms, floors, and deep-clean transformations.
- **Before & After (where shown)**: honest original-vs-completed pairs.
- **Search queries** for sourcing: `clean-kitchen`, `tidy-living-room`, `home-cleaning`, `spotless-bathroom`. Vary composition (wide/detail, premises/in-action).

## 6. Conversion

Category CTAs: **Get a Cleaning Quote**, **Book a Deep Clean**, and **Arrange a Callback**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
