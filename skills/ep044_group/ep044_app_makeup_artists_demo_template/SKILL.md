...
This skill adds only the makeup artists-specific content and imagery below. **A thin, image-poor screen is not acceptable** — the blueprint's definition of done applies. A full makeup artists website is expected: every common page above, populated with meaningful, category-accurate content.
...
- **Palette**: refined, editorial tones distinct from the existing nail/beauty builds (consider deep plum / soft bronze / warm neutral — check the registry). Read `epics/ep_044_web_apps/PALETTE_REGISTRY.md` first, propose 2–4 candidates genuinely distinct from every existing row, apply the chosen one via `:root` only, grep for stray hex outside `:root`, then append this site's row to the registry in the same turn.

## 4. Category content model — Makeup Artists

- **Services**: bridal makeup, occasion and party makeup, editorial and photoshoot makeup, group/party bookings, lessons and masterclasses, and trials.
- **Booking data** (feeds the quote/booking form and AI qualification): occasion, date and location, number of faces, style/look reference, skin type and allergies, trial required?, and travel/postcode.
- **Trust signals**: portfolio quality, training and hygiene, products/brands used, patch testing, punctuality for events, and travel policy. Use only honest, non-fabricated differentiators.
- **FAQs**: trials, travel, group bookings, how long a face takes, longevity, allergies/patch tests, and deposits.
- **Business model note**: usually mobile / travels to the client or works on location — lead with travel area and event availability rather than a fixed studio address (unless a studio exists).

## 5. Imagery — map the blueprint image matrix to makeup artists

Follow the blueprint's per-page image matrix and its mandatory sourcing/verification workflow (real photos only, verified HTTP 200 before use, no reuse across sibling sites, every `<img>` must load, descriptive alt text). Makeup Artists-specific subjects:

- **Hero (1)**: a beautifully finished makeup look (bridal or editorial reads best).
- **Two split/feature sections (1 each)**: **Bridal & occasions** (bridal, party and occasion makeup, group bookings, trials) and **Editorial & lessons** (photoshoot/editorial makeup, lessons and masterclasses), alternating image side.
- **Homepage gallery preview strip (4)** and **dedicated `gallery.html` (9–10, editorial masonry, captioned by sub-category)**: finished looks by style (bridal, natural, glam, editorial), close-up detail, and honest before-and-after transformations.
- **Before & After**: strong fit — tasteful, honest bare-face vs finished-look pairs (with permission).
- **Search queries** for sourcing: `makeup-artist`, `bridal-makeup`, `makeup-look`, `makeup-brushes`. Vary composition (wide/detail, in-action).

## 6. Conversion

Category CTAs: **Book a Trial**, **Check Your Date**, and **Send a Look Reference**. Keep the blueprint's conversion hierarchy (inspire → trust → explain → remove objections → request the enquiry) and a prominent phone number on mobile and desktop.
