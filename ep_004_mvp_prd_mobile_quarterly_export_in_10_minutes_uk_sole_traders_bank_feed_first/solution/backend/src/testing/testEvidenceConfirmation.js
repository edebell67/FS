...
async function runTest() {
  ...
  const capturedAt = "2026-03-21T09:30:00.000Z";
  ...
  const e1 = await ingestEvidence({
    store, userId, fileName: "tesco_receipt.png", type: "RECEIPT", content: "...", capturedAt
  });
  const e2 = await ingestEvidence({
    store, userId, fileName: "amazon_invoice.pdf", type: "INVOICE", content: "...", capturedAt
  });
  const e3 = await ingestEvidence({
    store, userId, fileName: "unknown_doc.jpg", type: "OTHER", content: "...", capturedAt
  });
  const e4 = await ingestEvidence({
    store, userId, fileName: "unprocessed_doc.jpg", type: "OTHER", content: "...", capturedAt
  });
  ...
  const csv = await generateEvidenceIndexCsv(store, userId, periodStart, periodEnd);
  
  // Checks
  const lines = csv.split("\n");
  const headers = lines[0].split(",");
  const e1Line = lines.find(l => l.includes(e1.evidence_id));
  const e2Line = lines.find(l => l.includes(e2.evidence_id));
  const e3Line = lines.find(l => l.includes(e3.evidence_id));

  if (!e1Line || !e2Line || !e3Line) {
    throw new Error("EvidenceIndex.csv is missing one or more expected evidence rows.");
  }

  const e1Row = e1Line.split(",");
  const e2Row = e2Line.split(",");
  const e3Row = e3Line.split(",");
