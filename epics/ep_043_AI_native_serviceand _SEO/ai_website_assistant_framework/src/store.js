...
export class JsonStore {
  ...
  async appendRecord(type, value) {
    ...
    this.records[type] = this.records[type].slice(0, RECORD_CAPS[type] || DEFAULT_RECORD_CAP);
    await this.persistRecords();
    return clone(record);
  }

  // Append several records of one type in a single persist — used for batched
  // analytics events so a page can flush many events with one write.
  async appendRecords(type, values) {
    if (!Object.hasOwn(this.records, type)) throw new Error("Unknown record type.");
    const now = new Date().toISOString();
    const created = values.map((value) => ({ id: randomUUID(), createdAt: now, ...clone(value) }));
    this.records[type].unshift(...created);
    this.records[type] = this.records[type].slice(0, RECORD_CAPS[type] || DEFAULT_RECORD_CAP);
    await this.persistRecords();
