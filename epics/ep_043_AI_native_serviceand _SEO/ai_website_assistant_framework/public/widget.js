(() => {
  ...
  // Share the analytics session id (aiw-session) with analytics-embed.js when
  // present, so a visitor's page behaviour and their chat join into one
  // journey inside the owner console's pageAnalytics summary; otherwise mint
  // one so the chat still has a session id of its own.
