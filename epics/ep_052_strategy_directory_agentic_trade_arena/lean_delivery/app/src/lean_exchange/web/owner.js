// VERSION HISTORY v1.3.0 · 2026-09-10 · Continuously refresh owner-agent messages while the workspace is connected.
// v1.2.1 · 2026-09-10 · Move Arena entry administration to the Arena screen.
// v1.1.1 · 2026-09-02 · Format exact zeros legibly and discard superseded portfolio/attribution responses.
...
(()=>{'use strict';
let session=0,token='',agents=[],messages=[],cursor=0,pending=null,positionRequest=0,attributionRequest=0,ownerTimer=null,refreshing=false;
...
function startOwnerUpdates(){clearInterval(ownerTimer);ownerTimer=setInterval(async()=>{if(refreshing)return;refreshing=true;try{await refresh();}catch(error){status(error.message,true);}finally{refreshing=false;}},5000);}
$('access').addEventListener('submit',async event=>{event.preventDefault();const button=event.submitter;button.disabled=true;session++;token=$('token').value.trim();try{const identity=await api('/v1/me');if(identity.role!=='owner')throw new Error('Use an owner credential, not an agent credential.');await refresh();$('identity').textContent='Owner '+identity.owner_id;$('token').value='';$('access').hidden=true;$('workspace').hidden=false;startOwnerUpdates();status('Connected. Messages, acknowledgements and replies update automatically.');}catch(error){token='';status(error.message,true);}finally{button.disabled=false;}});
$('refresh').onclick=async()=>{try{await refresh();status('Updated from the API.');}catch(error){status(error.message,true);}};
$('signout').onclick=()=>{clearInterval(ownerTimer);ownerTimer=null;session++;token='';agents=[];messages=[];cursor=0;pending=null;for(const id of ['agents','threads','positions','portfolio-totals','explain-agents','attribution'])$(id).replaceChildren();$('value-from').value='';$('value-to').value='';$('position-scope').textContent='';$('message').value='';$('identity').textContent='Not connected';$('workspace').hidden=true;$('access').hidden=false;status('Signed out. No credential retained.');};
