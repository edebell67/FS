# VERSION HISTORY v1.2.0 · 2026-09-10 · Assert the four-family board and its dedicated allowlisted stylesheet are delivered.
# v1.1.0 · 2026-09-02 · Assert restored3D canvas and audit switch use only existing allowlisted assets.
...
def test_arena_assets_are_allowlisted_and_observer_is_read_only():
    with TestClient(create_app()) as client:
        ...
        assert 'family-windows' in response.text and 'Strategy families' in response.text
        ...
        assert client.get('/assets/arena-families.css').status_code == 200
