/*
 * A C Automotive Engineering — demo site interactions.
...
(function () {
  ...
  const waHref = "https://wa.me/" + garageConfig.whatsapp;
  ...
  function renderBlog() {
    ...
    if (!el || typeof blogData === "undefined") return;
    el.innerHTML = blogData
      .map(
        (post) => `
      <article class="blog-card reveal">
        ...
        <p>${post.excerpt}</p>
        <div class="blog-meta">
          <span>${post.date}${post.placeholder ? " · sample" : ""}</span>
          <a href="#tool" aria-label="Use enquiry tool after reading ${post.title}">Ask about this ${icon("arrow-right")}</a>
        </div>
      </article>`
      )
      .join("");
  ...
  function thankYouHtml() {
    return `
      ...
      <h3>Thank you</h3>
      <p>Your enquiry has been prepared for <strong>${garageConfig.businessName}</strong>. A live version of this website can connect this form directly to the garage's email, CRM, WhatsApp, or booking system.</p>
  ...
  function initCookieBanner() {
    ...
    if (!banner) return;
    if (localStorage.getItem("ac_automotive_cookie_choice")) return;
    setTimeout(() => banner.classList.add("show"), 900);
    document.getElementById("cookieAccept")?.addEventListener("click", () => {
      localStorage.setItem("ac_automotive_cookie_choice", "accepted");
      banner.classList.remove("show");
    });
    document.getElementById("cookieDecline")?.addEventListener("click", () => {
      localStorage.setItem("ac_automotive_cookie_choice", "declined");
      banner.classList.remove("show");
    });
  }
