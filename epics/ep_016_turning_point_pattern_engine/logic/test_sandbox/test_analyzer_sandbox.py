...
# ---------------------------------------------------
STATE_ROOT = Path(r"C:\Users\edebe\eds\epics\ep_016_turning_point_pattern_engine\logic\test_sandbox\states")
STATE_ROOT.mkdir(exist_ok=True)
STATE_FILE = STATE_ROOT / f"state_{SYMBOL}_{datetime.now().strftime('%Y-%m-%d')}.json"

ORDER_DIR = Path(r"C:\Users\edebe\eds\epics\ep_016_turning_point_pattern_engine\logic\test_sandbox\ORDERS")
...
def load_state():
    ...
    global cumulative_net_pips, validated_net_pips, bucket_times, recent_trade_events
    if args.restart:
        if STATE_FILE.exists():
            try: STATE_FILE.unlink()
            except: pass
            print(f"Restart: Deleted previous state for {SYMBOL}.")
        return
    ...
    try:
        ...
        print(f"Successfully resumed state for {SYMBOL}. Total P&L: {cumulative_net_pips} pips.")
...
def fetch_quotes():
    try:
        with urlopen(HTTP_SOURCE_URL, timeout=5) as response:
            payload = {"data": [{"code": "GBP", "bid": 1.3320, "ask": 1.3322, "timestamp": datetime.now().isoformat()}]}
        return payload.get("data", [])
    except: return []

def compute_mid_metrics(symbol, bucket_time):
    open_bid = first_prices[symbol]["bid"].get(bucket_time)
    open_ask = first_prices[symbol]["ask"].get(bucket_time)
    if open_bid is None or open_ask is None: return None
    ...
    for side in ["bid", "ask"]:
        for price, bucket_counts in price_data[symbol][side].items():
            ...
            if count <= 0: continue
            if price > open_mid: above += count
            elif price == open_mid: at_open += count
            else: below += count
            
    return {"open": open_mid, "above": above, "at_open": at_open, "below": below, "net": above - below}
...
def display_frequency():
    ...
    left_lines = [
        f"--- Pattern Engine V2 (Sync) | {SYMBOL} | MODE: {mode_str} ---",
    ...
    if current_b:
        m = compute_mid_metrics(SYMBOL, current_b)
        if m:
            state, _ = classify_state(m, m, m["open"], (first_prices[SYMBOL]["bid"].get(prior_b) + first_prices[SYMBOL]["ask"].get(prior_b))/2 if prior_b else None)
            left_lines.append(f"\nSTATE: {ANSI_BRIGHT}{state}{ANSI_RESET} | MID_NET: {m['net']}")
            left_lines.append(f"  Mid Cluster: Open={m['open']:.5f} | Above={m['above']} | Below={m['below']} | Net={m['net']}")
...
def run_analyzer():
    global last_timestamp_str, bucket_times, cumulative_net_pips, validated_net_pips
    print(f"Initializing V2 Sync Analyzer for {SYMBOL}...")
    load_state()
    try:
        while True:
            should_render = False
            try:
                ...
                # --- 5-SECOND SYNCHRONIZED LOGIC PULSE (Matching Backtest) ---
                if ts_str != last_timestamp_str:
                    ...
                    if current_bid:
                        ...
                        m = compute_mid_metrics(SYMBOL, b_time)
                        if m:
                            prior_mid_open = (first_prices[SYMBOL]["bid"].get(prior_b) + first_prices[SYMBOL]["ask"].get(prior_b))/2 if prior_b else None
                            state, _ = classify_state(m, m, m["open"], prior_mid_open)
                            ...
                            if active and (FIXED_TP or FIXED_SL):
                                ...
                                if exit_reason:
                                    ...
                                    # Update validated P&L only if trade was live
                            ...
                            # B. Check Signal Changes
                            if last_trade_state_by_symbol.get(SYMBOL) != state:
                                if not active and state.startswith("LONG_") and "EXIT" not in state:
                                    # Target check applies to VALIDATED (Live) P&L
                                    if validated_net_pips < TARGET_PIPS:
                                        # GATED: Only Live if Total P&L is above floor
                                        is_live = (cumulative_net_pips >= LIVE_FLOOR)
                                        active_position_by_symbol[SYMBOL] = {"dir": "long", "entry_p": current_ask, "ts": ts_str, "is_live": is_live}
                                        ev = {"ts": ts_str, "symbol": SYMBOL, "event": "OPEN", "state": state, "price": current_ask}
                                        recent_trade_events.append(ev); append_trade_signal_log(ev)
                                        if is_live: generate_order_json(ev)
                                    else:
                                        print(f"{ANSI_BRIGHT}Daily Target Reached ({validated_net_pips}). No new trades.{ANSI_RESET}")
                                elif not active and state.startswith("SHORT_") and "EXIT" not in state:
                                    if validated_net_pips < TARGET_PIPS:
                                        is_live = (cumulative_net_pips >= LIVE_FLOOR)
                                        active_position_by_symbol[SYMBOL] = {"dir": "short", "entry_p": current_bid, "ts": ts_str, "is_live": is_live}
                                        ev = {"ts": ts_str, "symbol": SYMBOL, "event": "OPEN", "state": state, "price": current_bid}
                                        recent_trade_events.append(ev); append_trade_signal_log(ev)
                                        if is_live: generate_order_json(ev)
                                    else:
                                        print(f"{ANSI_BRIGHT}Daily Target Reached ({validated_net_pips}). No new trades.{ANSI_RESET}")
                                    ...
                                    should_close = False
                                    if active["dir"] == "long" and (state.startswith("SHORT_") or state == "EXIT_LONG"): should_close = True
                                    elif active["dir"] == "short" and (state.startswith("LONG_") or state == "EXIT_SHORT"): should_close = True
                                    if should_close:
                                        exit_p = current_bid if active["dir"] == "long" else current_ask
                                        mult = infer_pip_multiplier(SYMBOL)
                                        ...
                                        ev = {"ts": ts_str, "symbol": SYMBOL, "event": "CLOSE", "state": state, "price": exit_p, "net_pips": net_pips, "cum_pips": cumulative_net_pips}
                                        recent_trade_events.append(ev); append_trade_signal_log(ev)
                                        if active.get("is_live"): generate_order_json(ev)
                                        active_position_by_symbol.pop(SYMBOL)
                                last_trade_state_by_symbol[SYMBOL] = state
