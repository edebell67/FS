...
export default async function SitePreviewsPage({ searchParams }: { searchParams: Promise<{ error?: string; sent?: string; skipped?: string; reviewError?: string; reviewSent?: string; reviewSkipped?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl px-4 sm:px-6 py-8">
    ...
    <form action={sendSelectedPreviewLinksAction} className="mt-6">
      ...
      {initialCandidates.length > 0 && <section className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-5"><label className="flex items-start gap-2 text-sm"><input type="checkbox" name="confirmed" />I reviewed the selected recipients and preview URLs. Send the selected site preview links.</label><button className="mt-4 w-full rounded bg-brand-700 px-4 py-2 font-medium text-white sm:w-fit">Send site preview links</button></section>}
    </form>
    ...
    <section className="mt-12 border-t pt-8">
      ...
      <form action={sendOwnerReviewInvitationAction} className="mt-6">
        ...
        {reviewCandidates.length > 0 && <section className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-5"><label className="flex items-start gap-2 text-sm"><input type="checkbox" name="confirmed" />I reviewed the selected recipients and live preview URLs. Send a separate owner-review invitation with a new one-time link.</label><button className="mt-4 w-full rounded bg-brand-700 px-4 py-2 font-medium text-white sm:w-fit">Send owner-review invitation</button></section>}
      </form>
    </section>
  </main>;
}
