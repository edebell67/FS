...
export default async function ValidationAdminPage() {
  ...
  return <main className="mx-auto max-w-6xl px-6 py-12">
    ...
    <section className="mt-8 rounded-lg border p-4">
      ...
      <form action={setPartialPolicyAction} className="mt-3 flex items-center gap-3 text-sm">
    ...
    <section className="mt-8"><h2 className="text-lg font-semibold">Active rules</h2>
      <div className="mt-3 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b">
      ...
      <form action={saveValidationRuleAction} className="mt-4 grid gap-3 rounded-lg bg-slate-50 p-4 md:grid-cols-4">
        ...
        <button className="rounded bg-slate-900 px-4 py-2 text-sm font-medium text-white md:col-span-4">Add or replace rule</button>
      </form>
    </section>
    ...
    <section className="mt-10"><h2 className="text-lg font-semibold">Outstanding field repair queue</h2>
      <div className="mt-3 space-y-3">{queue.map((item) => <article key={item.outcomeId} className="rounded-lg border border-amber-200 p-4">
        ...
        <form action={applyFieldRepairAction} className="mt-3 grid gap-2 md:grid-cols-[1fr_1fr_auto]">
