...
export default async function VerificationBatchesPage() {
  ...
  const [businesses, batches] = await Promise.all([
    getEligibleVerificationBusinesses(), listVerificationBatches(),
  ]);
  return <main className="mx-auto max-w-6xl px-6 py-12">
    ...
    <p className="mt-2 text-slate-600">Select active businesses from the Validated pipeline. Preparation creates one secure active link per business and never sends a message.</p>
    <div className="mt-8"><VerificationBatchBuilder businesses={businesses}/></div>
