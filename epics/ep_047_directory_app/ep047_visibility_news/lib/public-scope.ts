...
export function publicScopeWhere(): SQL {
  return sql`(
    COALESCE((SELECT v.decision FROM public_business_visibility v WHERE v.business_id = ${businesses.id}), 'inherit') = 'show'
    OR (
      COALESCE((SELECT v.decision FROM public_business_visibility v WHERE v.business_id = ${businesses.id}), 'inherit') <> 'hide'
