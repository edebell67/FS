...
export function VerificationLinkPanel({ businessRef }: { businessRef: string }) {
  const [expiry, setExpiry] = useState(5);
  const [result, setResult] = useState<{ url: string; linkId: string; expiresAt: string } | null>(null);
  ...
  async function revoke() {
    ...
    if (response.ok) setResult(null); else setError("Unable to revoke link.");
  }
  return <section className="mt-8 rounded-xl border border-slate-200 p-5">
    ...
    {result && <div className="mt-4 rounded bg-amber-50 p-3 text-sm"><p className="font-medium">Copy-only preview — shown once</p>
      ...
      <button onClick={revoke} className="mt-2 text-sm font-medium text-red-700">Revoke link</button></div>}
  </section>;
}
