...
type Business = {
  ...
  town: string | null; stageLabel: string;
};
type PreparedItem = {
...
export function VerificationBatchBuilder({ businesses }: { businesses: Business[] }) {
  ...
  async function prepare() {
    ...
    try {
      const response = await fetch("/directoryadmin/api/verification-batches", {
        ...
        body: JSON.stringify({ businessIds: [...selected], expiresInDays: expiry }),
      });
    ...
    <div className="mt-5 overflow-x-auto"><table className="w-full text-left text-sm">
      <thead><tr className="border-b"><th className="p-2">
        <input aria-label="Select all eligible businesses" type="checkbox"
          ...
          onChange={(event) => setSelected(event.target.checked ? new Set(businesses.map((b) => b.id)) : new Set())}/>
      </th><th className="p-2">Business</th><th className="p-2">Validated stage</th><th className="p-2">Recipient route</th><th className="p-2">Send readiness</th></tr></thead>
      <tbody>{businesses.map((business) => <tr key={business.id} className="border-b">
        ...
        <td className="p-2">{business.stageLabel}</td>
        <td className="p-2">email: {business.email || "not configured"}</td>
        <td className="p-2">{business.email ? "individually ready" : "not ready"}</td>
      </tr>)}</tbody>
    </table>{businesses.length === 0 && <p className="p-6 text-center text-slate-500">No active businesses are currently eligible in Validated.</p>}</div>
  </section>;
}
