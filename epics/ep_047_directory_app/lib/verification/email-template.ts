export const VERIFICATION_TEMPLATE_VERSION = "verification-v3-direct-link";
export function renderVerificationEmail(input: {
  ...
  return {
    ...
    html: `<p>Please review the directory listing for ${escapeHtml(input.businessName)}.</p><p><a href="${escapeHtml(input.verificationUrl)}">Review and verify this listing</a></p><p>This secure link expires ${escapeHtml(input.expiresAt.toISOString())}. Submitting sends the details for manual review; it does not publish changes automatically.</p>${input.trackingPixelUrl ? `<img src="${escapeHtml(input.trackingPixelUrl)}" width="1" height="1" alt="" style="display:none" />` : ""}`,
  };
}
