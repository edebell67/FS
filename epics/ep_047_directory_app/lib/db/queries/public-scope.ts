...
export function publicScopeWhere(): SQL {
  return sql`(
    EXISTS (SELECT 1 FROM public_directory_settings s WHERE s.id = 'default'
      AND (s.town_mode = 'all' OR EXISTS (SELECT 1 FROM public_town_visibility t
        WHERE t.town_key = lower(trim(${businesses.town})) AND t.is_enabled)))
    AND EXISTS (SELECT 1 FROM public_directory_settings s WHERE s.id = 'default'
      AND (s.category_mode = 'all' OR EXISTS (SELECT 1 FROM public_category_visibility c
        WHERE c.category_key = lower(trim(${businesses.category})) AND c.is_enabled)))
