...
function render(state, payload) {
  ...
  const legalDisclaimer = payload.legalDisclaimer || {};
  ...
  app.innerHTML = `
    <div class="screen-shell">
      ...
      <main class="phone">
        ...
        <section class="legal-panel" data-testid="legal-disclaimer">
          <p class="section-label">${legalDisclaimer.eyebrow || "Release disclaimer"}</p>
          <h2>${legalDisclaimer.title || "Not tax advice"}</h2>
          <p>${legalDisclaimer.body || "This app helps organise the quarter-close workflow, but the export still needs user review."}</p>
          <strong>${legalDisclaimer.responsibility || "You remain responsible for checking the final export and keeping supporting records."}</strong>
        </section>
