...
def main():
    ...
    # local_strategies() also attaches open_trades/open_net_return (live
    # local-only fields, see app/repository.py) to each item dict, but the
    # Strategy pydantic model deliberately does NOT declare those fields
    # (see contracts.py version history) - Strategy.model_validate() below
    # just ignores unrecognized dict keys, so they never reach the
    # published snapshot or its hash. Local/Arena consumers that want this
    # data read the raw local_strategies()/local_open_trade_summary() dicts
    # directly instead, bypassing this contract entirely.
