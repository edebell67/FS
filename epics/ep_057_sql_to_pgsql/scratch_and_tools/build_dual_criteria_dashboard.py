...
# Load dual criteria json
json_path = r"C:\Users\edebe\.gemini\antigravity\brain\6aed791c-b3c5-4ab4-a571-c23206db4462\scratch\top10_dual_criteria_equity_data.json"
with open(json_path, "r", encoding="utf-8") as f:
    dual_data_json = f.read()

template = """<!DOCTYPE html>
<!--
 * top10_5min_equity_curves.html — Top 10 Strategy Models: Dual Ranking Criteria Selection (Net Return & Win Rate), 5-Min Directional Split, Replay from Baseline, Baseline Initializer & Delta Hiding
 *
 * VERSION HISTORY
 * v1.6.0 · 2026-09-20 · [V20260920_1650] Adds selectable Ranking Criteria: Top 10 by Net Return vs Top 10 by Win Rate; seamlessly applies all existing functionality (Replay Mode, Replay from Baseline, Directional Split modes, Delta line hiding, Model curve toggles, Win rate slider, and multi-date navigation) to both ranking criteria sets without losing any existing functionality.
...
<body class="p-2 sm:p-4 bg-slate-950 text-slate-100 antialiased min-h-screen">
  <div class="max-w-7xl mx-auto bg-slate-900/90 border border-slate-800 rounded-xl p-3 sm:p-6 shadow-2xl backdrop-blur-md">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-800">
      ...
      <!-- Controls (Ranking Criteria Selector, Date Selector, Split Mode) -->
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <div class="text-[11px] text-slate-400 mt-0.5 flex flex-wrap items-center gap-2">
            <span>Criteria: <b class="text-amber-400" id="activeCriteriaLabel">Top 10 Net Return</b></span>
            <span>&bull; Window: <b class="text-sky-300" id="activeDateLabel">Full Week</b></span>
    ...
    <div class="mt-4">
      <div class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 flex items-center justify-between font-mono">
        <span id="cardsGridTitle">Top 10 Models by Net Return for Selected Window</span>
  ...
  <script>
    // Dual Criteria Master Dataset
    const ALL_CRITERIA_DATA = __DUAL_DATA_JSON_PLACEHOLDER__;

    // Backward-compatible fallback for any direct readers
    const ALL_DATE_DATA = ALL_CRITERIA_DATA["NET_RETURN"];

    let currentCriteria = 'NET_RETURN'; // 'NET_RETURN' or 'WIN_RATE'
    ...
    // ==========================================
    // CRITERIA SELECTION (NET RETURN vs WIN RATE)
    // ==========================================
    function setRankingCriteria(criteria) {
      ...
      if (currentFrameIndex >= maxF) {
        currentFrameIndex = maxF - 1;
      }
      if (baselineFrameIndex >= maxF) {
    ...
    function updateCriteriaButtonsUI() {
    ...
    function updateDateLabelAndTitle() {
      const dObj = availableDates.find(d => d.id === currentDate);
      activeDateLabel.textContent = dObj ? (dObj.id === 'WEEK_OVERALL' ? 'Full Week (Sep 14–18)' : dObj.id) : currentDate;
      const critName = currentCriteria === 'NET_RETURN' ? 'Net Return' : 'Win Rate';
      cardsGridTitle.textContent = `Top 10 Models by ${critName} for ${activeDateLabel.textContent}`;
    }
    ...
    function startPlaybackLoop() {
      const intervalMs = Math.max(25, Math.round(BASE_INTERVAL_MS / replaySpeed));
    ...
    function updateRibbon() {
      ...
      if (splitMode === 'TRI_SPLIT') {
        ...
        ribbonModelName.textContent = `${critLabel} (${activeModels.length} of ${eligible.length})`;
        ...
        indicatorBar.style.backgroundColor = currentCriteria === 'NET_RETURN' ? '#38bdf8' : '#34d399';
...
full_html = template.replace("__DUAL_DATA_JSON_PLACEHOLDER__", dual_data_json)
...
print("SUCCESS: Updated top10_5min_equity_curves.html with selectable Ranking Criteria (Net Return / Win Rate)!")
