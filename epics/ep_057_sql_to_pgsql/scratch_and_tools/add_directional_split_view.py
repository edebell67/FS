...
<head>
  ...
  <title>Top 10 Strategy Models - Directional Split (Cum Net, Buy Net & Sell Net)</title>
      ...
      <div>
        <div class="flex items-center gap-2">
          ...
          <h1 class="text-xl sm:text-2xl font-black tracking-tight text-white font-mono">
            Top 10 Models: Directional Split View
          </h1>
        </div>
        <p class="text-xs text-slate-400 mt-1">
          Inspect Total Net, <span class="text-sky-400 font-semibold">Cum Buy Net (Longs)</span>, and <span class="text-amber-400 font-semibold">Cum Sell Net (Shorts)</span> across 5-minute intervals
        </p>
      </div>
    ...
    <div class="mt-3 p-3 bg-slate-950/70 border border-slate-800 rounded-xl flex flex-wrap items-center justify-between gap-3 font-mono">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <div class="text-[11px] text-slate-400 mt-0.5" id="ribbonSubtitle">
            Window: <span class="text-sky-300 font-semibold" id="activeDateLabel">Full Week</span> &bull; Click any model card below to inspect its individual Buy vs Sell split
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-5 text-right">
        <div>
          <div class="text-[9px] uppercase tracking-wider text-slate-400">Total Net</div>
        ...
        <div class="border-l border-slate-800 pl-3">
          <div class="text-[9px] uppercase tracking-wider text-sky-400">Cum Buy Net</div>
          <div class="text-xs sm:text-sm font-bold text-sky-400" id="ribbonBuyNet">+£0.00</div>
        </div>
        <div class="border-l border-slate-800 pl-3">
          <div class="text-[9px] uppercase tracking-wider text-amber-400">Cum Sell Net</div>
    ...
    <div class="mt-3 p-3 bg-slate-950/80 rounded-xl border border-slate-800/80 relative">
      ...
      <div class="flex flex-wrap items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-800/70 mt-2 font-mono">
        ...
        <div class="text-[11px] text-slate-500">
          5-Minute Intervals &bull; Sub-Ledger Directional Accounting
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    function updateRibbon() {
      ...
      if (splitMode === 'TRI_SPLIT') {
        ...
        ribbonTotNet.textContent = fmt(m.cum_net);
        ribbonTotNet.className = `text-base sm:text-lg font-black ${m.cum_net >= 0 ? 'text-emerald-400' : 'text-rose-400'}`;

        ribbonBuyNet.textContent = fmt(m.buy_net);
        ribbonBuyNet.className = `text-xs sm:text-sm font-bold ${m.buy_net >= 0 ? 'text-sky-400' : 'text-rose-400'}`;

        ribbonSellNet.textContent = fmt(m.sell_net);
        ribbonSellNet.className = `text-xs sm:text-sm font-bold ${m.sell_net >= 0 ? 'text-amber-400' : 'text-rose-400'}`;
      } else {
        const totNet = currentModels.reduce((acc, m) => acc + m.cum_net, 0);
        const totBuy = currentModels.reduce((acc, m) => acc + m.buy_net, 0);
        const totSell = currentModels.reduce((acc, m) => acc + m.sell_net, 0);
      ...
      currentModels.forEach((m, idx) => {
        ...
        card.innerHTML = `
          ...
          <div class="mt-2 pt-1 border-t border-slate-800/80 flex items-center justify-between text-[11px] font-mono">
            <span class="text-emerald-400 font-bold">Tot: ${fmt(m.cum_net)}</span>
            <span class="text-[10px] text-slate-400">
              <b class="text-sky-400">B:${fmt(m.buy_net)}</b> <b class="text-amber-400">S:${fmt(m.sell_net)}</b>
            </span>
          </div>
        `;
    ...
    function drawChart() {
      ...
      // Determine lines to draw based on splitMode
      if (splitMode === 'TRI_SPLIT') {
        const m = currentModels[selectedModelIndex] || currentModels[0];
        chartLegend.innerHTML = `
          <span class="flex items-center gap-1.5"><span class="w-3 h-0.5 bg-emerald-400 inline-block"></span><span class="text-emerald-300 font-bold">Total Net</span></span>
          <span class="flex items-center gap-1.5"><span class="w-3 h-0.5 bg-sky-400 inline-block"></span><span class="text-sky-300 font-bold">Cum Buy Net (Longs)</span></span>
          <span class="flex items-center gap-1.5"><span class="w-3 h-0.5 bg-amber-400 inline-block"></span><span class="text-amber-300 font-bold">Cum Sell Net (Shorts)</span></span>
        `;

        let vals = [];
        m.series.forEach(pt => {
          vals.push(pt.net, pt.buy, pt.sell);
        });
        ...
        // 1. Buy Net (Cyan)
        renderSingleLine(m.series, 'buy', '#38bdf8', 2.2, [4, 3], getX, getY);
        // 2. Sell Net (Amber)
        renderSingleLine(m.series, 'sell', '#fbbf24', 2.2, [4, 3], getX, getY);
        // 3. Total Net (Emerald solid)
        renderSingleLine(m.series, 'net', '#34d399', 3.0, [], getX, getY);

        // Labels at end
        const lastIdx = m.series.length - 1;
        const lastPt = m.series[lastIdx];
        drawPointLabel(getX(lastIdx), getY(lastPt.net), `Net: £${lastPt.net.toFixed(0)}`, '#34d399');
        drawPointLabel(getX(lastIdx), getY(lastPt.buy), `Buy: £${lastPt.buy.toFixed(0)}`, '#38bdf8');
        drawPointLabel(getX(lastIdx), getY(lastPt.sell), `Sell: £${lastPt.sell.toFixed(0)}`, '#fbbf24');
        ...
        // Multi-Model Mode (ALL_NET, BUY_NET, or SELL_NET)
        ...
        chartLegend.innerHTML = `<span>Displaying: <strong class="text-white">${metricName}</strong> for selected models</span>`;
        ...
        activeModels.forEach(m => {
          m.series.forEach(pt => allVals.push(pt[metricKey]));
        });

        let minVal = Math.min(0, ...allVals);
        let maxVal = Math.max(0, ...allVals);
        if (minVal === maxVal) { minVal -= 50; maxVal += 50; }
        ...
        activeModels.forEach(m => {
          renderSingleLine(m.series, metricKey, m.color, 2.2, [], getX, getY);
          if (m.series.length) {
            const lastIdx = m.series.length - 1;
            const lastVal = m.series[lastIdx][metricKey];
            drawPointLabel(getX(lastIdx), getY(lastVal), `${m.model} (£${lastVal.toFixed(0)})`, m.color);
          }
        });
    ...
    function renderSingleLine(series, key, color, lineWidth, dash, getX, getY) {
          ...
          tooltip.innerHTML = `
            ...
            <div class="space-y-1 font-mono text-[11px]">
              <div class="flex justify-between">
                <span class="text-emerald-400 font-semibold">Total Net:</span>
                <span class="font-bold ${pt.net >= 0 ? 'text-emerald-400' : 'text-rose-400'}">£${pt.net.toFixed(2)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-sky-400 font-semibold">Cum Buy Net:</span>
                <span class="font-bold ${pt.buy >= 0 ? 'text-emerald-400' : 'text-rose-400'}">£${pt.buy.toFixed(2)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-amber-400 font-semibold">Cum Sell Net:</span>
                <span class="font-bold ${pt.sell >= 0 ? 'text-emerald-400' : 'text-rose-400'}">£${pt.sell.toFixed(2)}</span>
              </div>
          ...
          activeModels.slice(0, 7).forEach(m => {
            ...
            rowsHtml += `
              <div class="flex justify-between items-center text-[10px] font-mono py-0.5">
                ...
                <span class="font-bold ${pt[metricKey] >= 0 ? 'text-emerald-400' : 'text-rose-400'}">£${pt[metricKey].toFixed(0)}</span>
              </div>
            `;
          });
...
print("Updated top10_5min_equity_curves.html with Directional Split View in both locations!")
