...
<head>
  ...
  <title>Top 10 Strategy Models - Directional Delta Hiding & Baseline Initializer</title>
...
<body class="p-3 sm:p-5 bg-slate-950 text-slate-100 antialiased min-h-screen">
  <div class="max-w-7xl mx-auto bg-slate-900/90 border border-slate-800 rounded-xl p-4 sm:p-6 shadow-2xl backdrop-blur-md">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-800">
      <div>
        ...
        <p class="text-xs text-slate-400 mt-1">
          Inspect Total Net, <span class="text-sky-400 font-semibold">Cum Buy Net (Longs)</span>, and <span class="text-amber-400 font-semibold">Cum Sell Net (Shorts)</span> &bull; Toggle any delta line on/off &bull; Tap chart to re-center curves from any timestamp
        </p>
      </div>
    ...
    <!-- Active Ribbon Inspector with Baseline Controls & Quick Visibility Toggles -->
    <div class="mt-3 p-3 bg-slate-950/70 border border-slate-800 rounded-xl flex flex-wrap items-center justify-between gap-3 font-mono">
      ...
      <!-- Quick Show All / Hide All Actions & Financial Metric Display -->
    ...
    <div class="mt-3 p-3 bg-slate-950/80 rounded-xl border border-slate-800/80 relative">
      ...
      <div class="flex flex-wrap items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-800/70 mt-2 font-mono gap-3">
        ...
        <div class="text-[11px] text-amber-300/90 flex items-center gap-1.5 ml-auto">
          ...
          <span>Tip: Click pill to <b>Hide / Show Delta</b> &bull; Tap chart to <b>Set Baseline Time</b></span>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    function updateRibbon() {
      ...
      if (splitMode === 'TRI_SPLIT') {
        ...
        ribbonModelName.textContent = `Active Models (${activeModels.length} of ${currentModels.length})`;
    ...
    function drawChart() {
      ...
      if (splitMode === 'TRI_SPLIT') {
        ...
        // In group modes (ALL_NET, BUY_NET, SELL_NET)
        // Determine active metric and verify if visible
        ...
        activeModels.forEach(m => {
          const basePt = getBasePoint(m.series);
          m.series.forEach(pt => allDeltas.push(pt[activeMetric] - basePt[activeMetric]));
        });
        ...
        activeModels.forEach(m => {
          const basePt = getBasePoint(m.series);
          renderDeltaLine(m.series, activeMetric, basePt[activeMetric], m.color, 2.2, [], getX, getY);
          if (m.series.length) {
            const lastIdx = m.series.length - 1;
            const deltaVal = m.series[lastIdx][activeMetric] - basePt[activeMetric];
...
print("SUCCESS: Updated top10_5min_equity_curves.html with Net/Buy/Sell Delta toggling in both locations!")
