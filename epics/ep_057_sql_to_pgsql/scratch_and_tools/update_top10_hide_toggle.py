    ...
    <div class="mt-3 p-3 bg-slate-950/80 rounded-xl border border-slate-800/80 relative">
      ...
      <div class="flex flex-wrap items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-800/70 mt-2 font-mono">
        ...
        <div class="text-[11px] text-amber-300/90 flex items-center gap-1.5">
          ...
          <span>Tip: Click eye icon on card to <b>Hide / Show</b> &bull; Tap chart to <b>Set Baseline Time</b></span>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    function renderCards() {
      ...
      currentModels.forEach((m, idx) => {
        ...
        card.onclick = (e) => {
          // If clicking toggle button specifically, handle toggle
          ...
          if (splitMode === 'TRI_SPLIT') {
            ...
            // Toggle visibility or isolate on card click
            if (visibleModels.has(m.model)) {
    ...
    function drawChart() {
      ...
      if (splitMode === 'TRI_SPLIT') {
        ...
        if (!activeModels.length) {
          ...
          ctx.fillText('All lines hidden. Click "Show All" or any card to display curves.', width / 2, height / 2);
          return;
        }
...
print("Updated top10_5min_equity_curves.html with Hide/Show toggles in both locations!")
