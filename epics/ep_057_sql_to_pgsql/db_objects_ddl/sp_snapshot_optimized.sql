...
-- =============================================================================
-- Stored Procedure: dbo.sp_snapshot_dna_model_summary_5min
-- Author:           Antigravity
-- Create Date:      2026-09-19
-- Version:          V20260919_1907
-- Description:      High-performance 5-minute periodic performance snapshot for
--                   all DNA models across closed and open trades.
-- =============================================================================
...
BEGIN
    ...
    DECLARE @date_start DATETIME2(3) = CAST(@date AS DATETIME2(3));
    DECLARE @date_end DATETIME2(3) = DATEADD(DAY, 1, @date_start);
    ...
    BEGIN
        IF EXISTS (
            SELECT 1 
            FROM dbo.tbl_dna_model_summary_snapshots_5min WITH (NOLOCK)
    ...
    -- 2. Aggregate Closed Trades using sargable range index
    ...
    WHERE c.created >= @date_start AND c.created < @date_end
      AND c.model LIKE 'DNA[_]%'
    GROUP BY c.model;

    CREATE UNIQUE CLUSTERED INDEX IX_cs_model ON #closed_stats(model);

    -- 3. Aggregate Open Trades (Live open positions)
    ...
    -- 4. Target active models (only models with open or closed activity)
    IF OBJECT_ID('tempdb..#active_models') IS NOT NULL DROP TABLE #active_models;
    SELECT DISTINCT model INTO #active_models
    FROM (
        SELECT model FROM #closed_stats
        UNION
        SELECT model FROM #open_stats
    ) a;

    CREATE UNIQUE CLUSTERED INDEX IX_am_model ON #active_models(model);

    -- 5. Insert Snapshot Records joined with product_forex for active models only
    ...
    LEFT JOIN #closed_stats cs ON cs.model = am.model
    LEFT JOIN #open_stats os ON os.model = am.model
    OUTER APPLY (
        SELECT TOP 1 product, strategy_name, dna_json
        FROM dbo.product_forex pf WITH (NOLOCK)
        WHERE pf.model = am.model
    ) pf;

    SET @rows_inserted = @@ROWCOUNT;

    -- Cleanup
    DROP TABLE IF EXISTS #closed_stats, #open_stats, #active_models;
