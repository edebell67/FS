...
const confirmCompositionInternal = async (compositionId, options = {}) => {
  ...
  const requestNow = options.now instanceof Date ? options.now : new Date(options.now || Date.now());
  ...
  try {
    ...
    if (requestedCommitMode === 'auto') {
      const eligibility = await evaluateAutoCommitEligibility(client, {
        ...
        amount: nextState.amount ?? monetaryDetails.amount ?? monetaryDetails.gross_amount,
