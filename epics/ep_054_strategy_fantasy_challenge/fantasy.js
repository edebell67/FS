(() => {
  ...
  $('openFinder').onclick=()=>{window.open('http://172.22.108.121:8054/','_blank','noopener'); const next=candidates[0];if(!state.strategies.some(s=>s.id===next.id)){state.strategies.push(next);state.provenance='FINDER_AI';render();}event('FINDER_OPENED_FROM_FANTASY','Simulated Finder match added; existing Finder AI opened in a new tab.');};
