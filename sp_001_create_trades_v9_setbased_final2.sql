...
BEGIN
  ...
  SELECT NEWID(), pf.pf_ord, pf.product, pf.model, pf.product_type,
         @now, pf.commission, 
         CASE WHEN pf.target_profit IS NULL THEN 10 ELSE pf.target_profit END,  -- Handle NULL target_profit
         CASE WHEN pf.target_loss IS NULL THEN -5 ELSE pf.target_loss END,      -- Handle NULL target_loss
         pf.trade_qty,
         N'buy',
  ...
  SELECT NEWID(), pf.pf_ord, pf.product, pf.model, pf.product_type,
         @now, pf.commission, 
         CASE WHEN dna.tp_pips IS NULL THEN 10 ELSE dna.tp_pips END,  -- Handle NULL tp_pips
         CASE WHEN dna.sl_pips IS NULL THEN -5 ELSE dna.sl_pips END,   -- Handle NULL sl_pips
         CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END, -- Handle NULL leg_qty
         CASE WHEN dna.side_pref = 'BUY' THEN N'buy' ELSE N'sell' END,
         CASE WHEN dna.side_pref = 'BUY' THEN q.askPrice ELSE q.bidPrice END,
         CASE WHEN dna.side_pref = 'BUY' THEN q.askPrice ELSE q.bidPrice END,
         CASE WHEN dna.side_pref = 'BUY' THEN ((q.bidPrice - q.askPrice) * CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END) - pf.commission ELSE ((q.bidPrice - q.askPrice) * CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END) - pf.commission END,
         CASE WHEN dna.side_pref = 'BUY' THEN ((q.askPrice - q.bidPrice) * CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END) - pf.commission ELSE ((q.askPrice - q.bidPrice) * CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END) - pf.commission END,
         CASE
           WHEN @wl_all = 1 THEN 1
           ELSE CASE WHEN CHARINDEX(','+LOWER(pf.product)+',', ','+@tpl_clean+',') > 0 THEN 1 ELSE 0 END
         END
  ...
  WHERE pf.trade_freq = 99 
    ...
    AND ISNULL(oq.open_qty, 0) + CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END <= @max_open_trade_qty
    AND (SELECT COUNT(*) FROM dbo.combined_trades_open WHERE model = pf.model) < CASE WHEN dna.max_legs IS NULL THEN 1 ELSE dna.max_legs END;
  ...
  SELECT NEWID(), pf.pf_ord, pf.product, pf.model, pf.product_type,
         ...
         CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END, -- Handle NULL leg_qty
         LOWER(ssc.side),
         CASE WHEN ssc.side = 'BUY' THEN q.askPrice ELSE q.bidPrice END,
         CASE WHEN ssc.side = 'BUY' THEN q.askPrice ELSE q.bidPrice END,
         CASE WHEN ssc.side = 'BUY' THEN ((q.bidPrice - q.askPrice) * CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END) - pf.commission ELSE ((q.bidPrice - q.askPrice) * CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END) - pf.commission END,
         CASE WHEN ssc.side = 'BUY' THEN ((q.askPrice - q.bidPrice) * CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END) - pf.commission ELSE ((q.askPrice - q.bidPrice) * CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END) - pf.commission END,
         CASE
           WHEN @wl_all = 1 THEN 1
           ELSE CASE WHEN CHARINDEX(','+LOWER(pf.product)+',', ','+@tpl_clean+',') > 0 THEN 1 ELSE 0 END
         END
  ...
  JOIN #q q ON q.product = pf.product
  JOIN dbo.vw_product_forex_dna_expanded dna ON dna.product = pf.product AND dna.model = pf.model
  JOIN dbo.strategy_side_current ssc ON ssc.product = pf.product AND ssc.model = pf.model
  LEFT JOIN #open_qty oq ON oq.model = pf.model
  WHERE pf.trade_freq = 99 
    AND dna.side_pref = 'BOTH'
    AND ssc.side IS NOT NULL
    AND ISNULL(oq.open_qty, 0) + CASE WHEN dna.leg_qty IS NULL THEN 10000 ELSE dna.leg_qty END <= @max_open_trade_qty
    AND (SELECT COUNT(*) FROM dbo.combined_trades_open WHERE model = pf.model) < CASE WHEN dna.max_legs IS NULL THEN 1 ELSE dna.max_legs END;
