...
BEGIN
  ...
  -- Debug: Check for NULLs in #pf
  DECLARE @null_count INT;
  SELECT @null_count = COUNT(*) FROM #pf WHERE target_profit IS NULL OR target_loss IS NULL;
  SET @nowtxt = CONVERT(varchar(23), GETDATE(), 121);
  ;RAISERROR(N'[%s] Debug: NULL count in #pf = %d', 0, 1, @nowtxt, @null_count) WITH NOWAIT;
