...
BEGIN
  ...
  SELECT NEWID(), pf.pf_ord, pf.product, pf.model, pf.product_type,
         @now, pf.commission, 
         ISNULL(dna.tp_pips, 10),  -- Handle NULL tp_pips
         ISNULL(dna.sl_pips, -5),   -- Handle NULL sl_pips
         ISNULL(dna.leg_qty, 10000), -- Handle NULL leg_qty
         CASE WHEN dna.side_pref = 'BUY' THEN N'buy' ELSE N'sell' END,
         CASE WHEN dna.side_pref = 'BUY' THEN q.askPrice ELSE q.bidPrice END,
         CASE WHEN dna.side_pref = 'BUY' THEN q.askPrice ELSE q.bidPrice END,
         CASE WHEN dna.side_pref = 'BUY' THEN ((q.bidPrice - q.askPrice) * ISNULL(dna.leg_qty, 10000)) - pf.commission ELSE ((q.bidPrice - q.askPrice) * ISNULL(dna.leg_qty, 10000)) - pf.commission END,
         CASE WHEN dna.side_pref = 'BUY' THEN ((q.askPrice - q.bidPrice) * ISNULL(dna.leg_qty, 10000)) - pf.commission ELSE ((q.askPrice - q.bidPrice) * ISNULL(dna.leg_qty, 10000)) - pf.commission END,
         CASE
           WHEN @wl_all = 1 THEN 1
           ELSE CASE WHEN CHARINDEX(','+LOWER(pf.product)+',', ','+@tpl_clean+',') > 0 THEN 1 ELSE 0 END
         END
  ...
  WHERE pf.trade_freq = 99 
    ...
    AND ISNULL(oq.open_qty, 0) + ISNULL(dna.leg_qty, 10000) <= @max_open_trade_qty
    AND (SELECT COUNT(*) FROM dbo.combined_trades_open WHERE model = pf.model) < ISNULL(dna.max_legs, 1);
  ...
  SELECT NEWID(), pf.pf_ord, pf.product, pf.model, pf.product_type,
         ...
         CASE WHEN ssc.side = 'BUY' THEN q.askPrice ELSE q.bidPrice END,
         CASE WHEN ssc.side = 'BUY' THEN ((q.bidPrice - q.askPrice) * ISNULL(dna.leg_qty, 10000)) - pf.commission ELSE ((q.bidPrice - q.askPrice) * ISNULL(dna.leg_qty, 10000)) - pf.commission END,
         CASE WHEN ssc.side = 'BUY' THEN ((q.askPrice - q.bidPrice) * ISNULL(dna.leg_qty, 10000)) - pf.commission ELSE ((q.askPrice - q.bidPrice) * ISNULL(dna.leg_qty, 10000)) - pf.commission END,
         CASE
           WHEN @wl_all = 1 THEN 1
           ELSE CASE WHEN CHARINDEX(','+LOWER(pf.product)+',', ','+@tpl_clean+',') > 0 THEN 1 ELSE 0 END
         END
