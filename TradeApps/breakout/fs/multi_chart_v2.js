...
function getCurrentWorkflowImportProductTypeV2(payload) {
...
function buildWorkflowImportPresetNameV2(baseName, productType) {
...
function getMetricValueV2(pt, metric) {
    if (!pt) return 0;
...
function buildSteppedSeriesV2(series, metric, startMs, axisMaxMs) {
    let baseline = 0;
    let lastVal = 0;
    const data = [{ x: startMs, y: 0 }];
    for (const pt of series) {
        const val = getMetricValueV2(pt, metric);
        if (pt.ms <= startMs) {
            baseline = val;
            continue;
        }
...
/**
 * [V20260203_1325] Add Remaining Strategies (Exact Set of 4) for V2
...
function addRelatedStrategies(groupName) {
    ...
    prefixes.forEach(prefix => {
        ...
        const key = `${targetName}|${product}`; // Check processedSeries key (no spaces)

        if (processedSeries[key]) {
            const overlayKey = `${targetName} | ${product}`; // Display key (spaces)
...
function updateCharts(filterTime = null) {
    ...
    let axisMaxMs = (filterTime || playbackTime || datasetLatestTime || endOfDay).getTime();
    if (axisMaxMs < startMs) axisMaxMs = startMs;
    ...
    orderedGroupNames.forEach((groupName, index) => {
        ...
        configs.forEach(config => {
            ...
            const val = getMetricValueV2(lastPt, metric);
        ...
        const datasets = configs.map(config => {
            ...
            const series = processedSeries[lookupKey] || [];
            const metric = config.metric || globalMetric;
            const steppedData = buildSteppedSeriesV2(series, metric, startMs, axisMaxMs);
...
function renderGroupChartsV2({ groupName, card, configs, datasets, startMs, axisMaxMs, leaderKey, isGroupLive }) {
    const netContainer = card.querySelector('.chart-container.net-view');
    if (netContainer) netContainer.style.display = 'flex';
    renderNetChartV2(groupName, card, datasets, startMs, axisMaxMs);
}

function renderNetChartV2(groupName, card, datasets, minX, maxX) {
    const canvas = card.querySelector(`#group-chart-${groupName}-net`);
    renderChartOnCanvasV2(groupName, 'net', canvas, datasets, minX, maxX);
}



function renderChartOnCanvasV2(groupName, slot, canvas, datasets, minX, maxX) {
    if (!canvas) return;
...
function createGroupCard(groupName) {
    ...
    // [V20260126_1230] Live Mode Toggle Sync
    const liveConfig = activeOverlays.find(o => o.group === groupName && o.isLive);

    // [V20260131_0410] Sim Mode Logic
    const modeEl = document.getElementById('runMode');
    const isSim = modeEl && modeEl.value === 'sim';

    const liveClass = liveConfig ? (isSim ? 'active sim' : 'active') : '';
    const liveText = liveConfig ? (isSim ? 'SIM: ON' : 'LIVE: ON') : 'LIVE: OFF';
    const splitState = getGroupSplitState(groupName);
    const splitButton = splitState
        ? `<button class="action-btn split-toggle" onclick="handleSplitComboAction(event, '${groupName}')" title="${splitState.label}">
               <i class="fas ${splitState.icon}"></i> ${splitState.label}
           </button>`
        : '';
    const isLive = liveConfig ? true : false; // Determine if any overlay in the group is live
...
function updateDatasetTimeBounds() {
    ...
    // [V20260203_1320] Start at the Hour of the earliest data
    let earliest = null;
    let latest = null;
    ...
    datasetLatestTime = latest ? new Date(latest) : endOfDay;
    updateStartTimeUI();
}
...
function applyRankSelection() {
    ...
    for (const key in processedSeries) {
        ...
        // Use the total trades this model performed during the session for the filter
        const totalTradesForDay = series[series.length - 1].trade_count;
        if (totalTradesForDay < minTrades) continue;
        ...
        if (delta > 0) {
            candidates.push({ key: key.replace('|', ' | '), delta: delta });
        }
    }

    // Always top for this view (v2)
...
async function loadView(name) {
    ...
    const views = JSON.parse(localStorage.getItem('pnl_graph_views_multi_v2') || '{}'), v = views[name];
    if (v) {
        // [V20260212_1655] Backward compatibility: infer date/time from legacy preset names: MMDD_HHMM_...
        let inferredDate = null;
        let inferredHms = null;
        const legacyMatch = String(name).match(/^(\d{2})(\d{2})_(\d{2})(\d{2})_/);
        if (legacyMatch) {
            const yyyy = String(new Date().getFullYear());
            inferredDate = `${yyyy}-${legacyMatch[1]}-${legacyMatch[2]}`;
            inferredHms = `${legacyMatch[3]}:${legacyMatch[4]}:00`;
        }

        // [V20260212_1645] Restore preset date first, then refetch data so series context matches creation day.
        const dateEl = document.getElementById('tradeDate');
        const savedDate = v.trade_date || inferredDate || null;
        ...
        // [V20260212_1630] Restore the saved start-time for accurate preset replay.
        const savedHms = v.start_time_hms || (v.start_time ? String(v.start_time).substring(11, 19) : null) || inferredHms;
        if (savedHms && document.getElementById('tradeDate').value) {
            currentStartTime = new Date(`${document.getElementById('tradeDate').value}T${savedHms}Z`);
            isStartTimeCustom = true;
            updateStartTimeUI();
        }
        // Ensure playback cursor is never before restored start-time.
        const endBound = getPlaybackBoundaries().endOfDay;
        playbackTime = (currentStartTime && endBound < currentStartTime) ? new Date(currentStartTime.getTime()) : endBound;
        updatePlaybackUI();
        updateCharts();
    }
}
function saveCurrentView() {
    ...
    const views = JSON.parse(localStorage.getItem('pnl_graph_views_multi_v2') || '{}');
    const activeStart = getActiveStartTime();
    views[name] = {
        overlays: activeOverlays,
        metric: document.getElementById('metricSelector').value,
        trade_date: document.getElementById('tradeDate').value || null,
        start_time: activeStart ? activeStart.toISOString() : null,
        start_time_hms: activeStart ? activeStart.toISOString().substring(11, 19) : null
    };
    localStorage.setItem('pnl_graph_views_multi_v2', JSON.stringify(views));
...
function saveToBucket(groupName) {
    ...
    // [V20260212_1635] Keep bucket and preset aligned so preset reload reproduces the original chart setup.
    try {
        const views = JSON.parse(localStorage.getItem('pnl_graph_views_multi_v2') || '{}');
        views[bucketName] = {
            overlays: overlays,
            metric: localGlobalMetric,
            trade_date: document.getElementById('tradeDate').value || null,
            start_time: activeStart ? activeStart.toISOString() : null,
            start_time_hms: activeStart ? activeStart.toISOString().substring(11, 19) : null
        };
        ...
        console.warn("Failed to mirror bucket into presets:", e);
    }
...
async function populateSavedViews() {
    const sel = document.getElementById('loadViewDropdown'), views = JSON.parse(localStorage.getItem('pnl_graph_views_multi_v2') || '{}');
...
function submitTransfer() {
    ...
    // Preserve chart start-time context for transferred preset if absent.
    if (!views[targetName].start_time_hms) {
        const activeStart = getActiveStartTime();
        views[targetName].trade_date = document.getElementById('tradeDate').value || null;
        views[targetName].start_time = activeStart ? activeStart.toISOString() : null;
        views[targetName].start_time_hms = activeStart ? activeStart.toISOString().substring(11, 19) : null;
    }
    localStorage.setItem('pnl_graph_views_multi_v2', JSON.stringify(views));
    activeOverlays = activeOverlays.filter(o => o.group !== groupName);
...
/**
 * [V20260126_1225] Trade Drilldown Logic
 */
    ...
    overlays.forEach(o => {
        ...
        series.forEach(pt => {
            if (pt.ms <= axisMaxMs) {
                ...
                const indivReturn = isAlt ? (pt.net_return * -1) : (pt.net_return || 0);
...
function renderDrilldownTable() {
    ...
    currentDrilldownTrades.forEach(t => {
        ...
        const detailId = `mcv2_${++drilldownTradeDetailSeq}`;
...
function sortDrilldownTable(colIdx, force = false) {
    ...
    const keys = ['ms', 'model', 'type', 'signal', 'result', 'total', 'exit', 'status', 'live'];
...
function peekSummaryImportPayload() {
    const raw = localStorage.getItem('multi_chart_v2_import_payload');
    if (!raw) return null;
    try {
        const payload = JSON.parse(raw);
        if (payload && Array.isArray(payload.items) && payload.items.length > 0) return payload;
    } catch (e) {
        console.warn('Invalid multi_chart_v2 import payload', e);
        localStorage.removeItem('multi_chart_v2_import_payload');
    }
    return null;
}
...
function consumeSummaryImportPayload() {
    ...
    const currentProductType = getCurrentWorkflowImportProductTypeV2(payload);
...
async function consumeWorkflowImportPayload() {
    try {
        ...
        if (runId) {
            const lastRunId = localStorage.getItem(`multi_chart_v2_workflow_import_run_id_${currentProductType}`) || '';
            if (lastRunId === runId) return;
            localStorage.setItem(`multi_chart_v2_workflow_import_run_id_${currentProductType}`, runId);
        }
        localStorage.setItem('multi_chart_v2_import_payload', JSON.stringify(payload));
        const result = consumeSummaryImportPayload() || { added: 0, groups: [] };
        if (result.added > 0) {
            const presetName = buildWorkflowImportPresetNameV2(
            ...
            if (overlaysForPreset.length > 0) {
                ...
                const lastSig = localStorage.getItem(`multi_chart_v2_workflow_last_signature_${currentProductType}`) || '';
                if (signature === lastSig) {
                    console.log('[MULTI-CHART-V2-WF-IMPORT] Skipped preset save (identical workflow chart set).');
                    return;
                }
                const views = JSON.parse(localStorage.getItem('pnl_graph_views_multi_v2') || '{}');
                views[presetName] = {
                ...
                localStorage.setItem('pnl_graph_views_multi_v2', JSON.stringify(views));
                localStorage.setItem(`multi_chart_v2_workflow_last_signature_${currentProductType}`, signature);
                populateSavedViews();
            }
        }
    } catch (e) {
        console.warn('[MULTI-CHART-V2-WF-IMPORT] Failed:', e);
    }
}
...
async function initMultiChartV2() {
    initEventHandlers();
    const pendingImport = peekSummaryImportPayload();
    if (pendingImport && pendingImport.date && document.getElementById('tradeDate')) {
        document.getElementById('tradeDate').value = pendingImport.date;
    }
    await fetchData();
    populateSavedViews();
    await loadGridLiveStatus();
...
// [V20260126_1230] Sync Live status from Backend
...
// [V20260126_1230] Live Grid Trading Toggle
