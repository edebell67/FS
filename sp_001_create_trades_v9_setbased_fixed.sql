...
BEGIN
  ...
  CREATE TABLE #cand(
     ...
     target_profit smallint NULL,  -- Changed to allow NULL
     target_loss smallint NULL,    -- Changed to allow NULL
  ...
  INSERT #cand
  SELECT NEWID(), pf.pf_ord, pf.product, pf.model, pf.product_type,
         @now, pf.commission, dna.tp_pips, dna.sl_pips, dna.leg_qty,
  ...
  WHERE pf.trade_freq = 99 
    ...
    AND (SELECT COUNT(*) FROM dbo.combined_trades_open WHERE model = pf.model) < dna.max_legs;
  ...
  SELECT NEWID(), pf.pf_ord, pf.product, pf.model, pf.product_type,
         ...
         CASE WHEN ssc.side = 'BUY' THEN q.askPrice ELSE q.bidPrice END,
         CASE WHEN ssc.side = 'BUY' THEN ((q.bidPrice - q.askPrice) * dna.leg_qty) - pf.commission ELSE ((q.bidPrice - q.askPrice) * dna.leg_qty) - pf.commission END,
         CASE WHEN ssc.side = 'BUY' THEN ((q.askPrice - q.bidPrice) * dna.leg_qty) - pf.commission ELSE ((q.askPrice - q.bidPrice) * dna.leg_qty) - pf.commission END,
         CASE
           WHEN @wl_all = 1 THEN 1
           ELSE CASE WHEN CHARINDEX(','+LOWER(pf.product)+',', ','+@tpl_clean+',') > 0 THEN 1 ELSE 0 END
         END
  ...
  /* === REPLACE your current unconditional insert with this === */
	...
	SELECT
		...
		c.commission,
		ISNULL(c.target_profit, 0),  -- Use ISNULL to handle NULLs
		ISNULL(c.target_loss, 0),    -- Use ISNULL to handle NULLs
		N'F',
		0, -- tradeable
		c.net_return,
		c.alt_net_return,
