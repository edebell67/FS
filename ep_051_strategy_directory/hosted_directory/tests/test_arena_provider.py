# VERSION HISTORY
# v1.0.1 · 2026-09-04 · Updates the windowed-query assertion to cover the deliberate repository-backed behavior used by hosted PostgreSQL rather than the retired SQL-Server-only rejection.
...
def test_windowed_query_on_repository_backend_returns_a_valid_result(tmp_path):
    """Hosted repository backends accept a requested time window rather than
    rejecting it through the retired SQL-Server-only 503 gate."""
    app = create_app(repository=MemoryRepository(), settings=Settings(data_backend="memory", ep052_intelligence_token=TOKEN,
                      arena_deliveries_path=str(tmp_path / "d.sqlite")))
